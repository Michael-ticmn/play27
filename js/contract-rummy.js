...
function renderSeats(round) {
  ...
  opponents.forEach((opp, i) => {
    ...
    for (let j = 0; j < Math.min(cardsInHand, 14); j++) {
      cardBacks += '<div class="opp-card-back"><img src="assets/card-back.svg?v=0.8.6"></div>';
    }
...
function flyCardToHand(sourceRect, cardCode, isFaceDown, isPenalty) {
  return new Promise(resolve => {
    ...
    if (isFaceDown) {
      flyEl.innerHTML = '<img src="assets/card-back.svg?v=0.8.6" style="width:100%;height:100%;border-radius:6px;">';
    } else if (cardCode) {
